<?php
  ...
  if(isset($_POST['quantidade']) && empty($_POST['quantidade']) == false){
    ...
    $produtos = addslashes($_POST['select2']);
    $quantidade = addslashes($_POST['quantidade']);

    
    
    $sql = "INSERT INTO produtovenda(`venda`, `produto`, `quantidade`, `valor`) VALUES ((SELECT MAX(id) FROM vendas WHERE numero = $numero),$produtos,$quantidade,(SELECT valor FROM produtos WHERE id = $produtos))";  
  ...
  <head>
    ...
    <div class="jumbotron">
      <form method = "POST">
        <h2>Nova Venda</h2>         
        Número da venda:<br/>
        <input type="text" class="form-control" style="width: 25%;" name="numero" />
        <!--Produtos:<br/>
        <input type="text" class="form-control" style="width: 25%;" name="produtos" required="true"/> -->
        Quantidade:<br/> 
        <input type="text" class="form-control" style="width: 15%;" name="quantidade" required="true"/>
...
<?php 
      ...
      echo 'Produto:';
      echo '<select class="form-control" name="select2" style="width: 17%;">';
...
?>
        ...
        <input class="btn btn-success" type="submit" value = "&#10003 Cadastrar Venda" />
        <a href="inicial-administrador.php" class="btn btn-danger">&#10005 Cancelar</a>  
      </form>
  </div>
