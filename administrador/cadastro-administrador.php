...
<html lang="pt-br">
  ...
  <?php
    ...
    if($sql->rowCount() == 0){
      ...
      echo '<strong>Aviso!</strong> Você ainda não possui administradores cadastrados.';
      echo '</div>';
  ...
  ?>
        ...
        <?php
          ...
          if($sql->rowCount() > 0){
            ...
            foreach ($sql-> fetchAll() as $administrador) {
                    echo '<td rowspan="2">'.$administrador['id'].'</td>';
                    echo'<td>'.$administrador['nome']. '</td>';
                    echo'<td>'.$administrador['telefone'].'</td>';
                    echo'<td>'.$administrador['endereco'].'</td>';
                    echo'<td>'.$administrador['cpf'].'</td>';
                    echo'<td>'.$administrador['dataNasc'].'</td>';
                    echo'<td>R$ '.$administrador['salario'].'</td>';
                    echo '<td><a href="editar-administrador.php?id='.$administrador['id'].'" class="btn btn-warning">&#9999; Editar</a></td>';
                    echo '<td><a href="excluir-administrador.php?id='.$administrador['id'].'" class="btn btn-danger">&#10006; Excluir</a></td>';
                echo '</tbody>';
              echo '</tr>';
            } 
          } 
      ...
      <a href="adicionar-administrador.php" class="btn btn-success">&#10010; Novo Administrador</a>
  ...
  <br>
      ...
      <div class="jumbotron">
        ...
        <p>Esta página é responsável pela manutenção dos administradores do sistema Lancho'System.</p>
      </div>
