...
<html lang="pt-br">
  ...
  <?php
    ...
    if($sql->rowCount() == 0){
      ...
      echo '<strong>Aviso!</strong> Você ainda não possui clientes cadastrados.';
      echo '</div>';
        ...
        <?php
          ...
          if($sql->rowCount() > 0){
            ...
            foreach ($sql-> fetchAll() as $clientes) {
                    echo '<td rowspan="2">'.$clientes['id'].'</td>';
                    echo'<td>'.$clientes['nome']. '</td>';
                    echo'<td>'.$clientes['endereco'].'</td>';
                    echo'<td>'.$clientes['cpf'].'</td>';
                    echo'<td>'.$clientes['dataNasc'].'</td>';
                    echo '<td><a href="editar-cliente.php?id='.$clientes['id'].'" class="btn btn-warning">&#9999; Editar</a></td>';
                    echo '<td><a href="excluir-cliente.php?id='.$clientes['id'].'" class="btn btn-danger">&#10006; Excluir</a></td>';
                echo '</tbody>';
              echo '</tr>';
            } 
          } 
  ...
  <br>
      ...
      <div class="jumbotron">
        ...
        <p>Esta página é responsável pela manutenção dos clientes do sistema Lancho'System.</p>
      </div>
